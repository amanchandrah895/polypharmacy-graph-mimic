\begin{table*}[t]
\centering
\caption{AUROC (95\% bootstrap CI), AUPRC, F1, and Brier score for every outcome $\times$ feature-set $\times$ model combination. delirium\_label / tabular rows are the leakage-corrected refit; pre-correction AUROC (before excluding delirium-defining drug columns) shown alongside for transparency (see Section~\ref{sec:methods}).}
